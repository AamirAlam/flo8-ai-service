...
openai_func = get_registry().get("openai").create(name="text-embedding-3-small", apiKey="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")
...
def retrieve_similar_docs(table: LanceTable, query: str, query_type: str = 'hybrid', limit: int = 100, reranker_weight: float = 0.7):
    """
    Retrieve documents from the LanceDB table using a hybrid approach
    (semantic + full-text search), then rerank them.
    """
